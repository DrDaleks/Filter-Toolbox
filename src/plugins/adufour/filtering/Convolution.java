	...
	public static void convolve(Sequence input, Sequence kernel, boolean zeroEdges, int nbIter, EzVarBoolean stopFlag)
	{
		...
		convolution: for (int t = 0; t < input.getSizeT(); t++)
		{
			for (int c = 0; c < input.getSizeC(); c++)
			{
				...
				{
					Array1DUtil.arrayToDoubleArray(input.getDataXY(t, z, c), _inZXY[z], input.isSignedDataType());
				}
				...
				for (int i = 0; i < nbIter; i++)
				{
					for (int z = 0; z < _inZXY.length; z++)
					{
						...
						Array1DUtil.doubleArrayToSafeArray(_outXY, input.getDataXY(t, z, c), input.isSignedDataType());
					} // end for(z)
